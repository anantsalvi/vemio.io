    ...
    >
      ...
      <div className="alert-meta">
        ...
        <span className={`alert-state ${
          ...
          alert.state === 'resolved' ? 'alert-state--resolved' : 'alert-state--dim'
        }`}>
      ...
      {/* Audit trail */}
      {(alert.acknowledged_by || alert.resolved_by || alert.glpi_ticket_id) && (
        <div className="alert-audit">
          {alert.acknowledged_by && (
            <span className="alert-audit-item">
              Acknowledged by <strong>{alert.acknowledged_by}</strong>
              {alert.acknowledged_at && ` · ${getTimeAgo(alert.acknowledged_at)}`}
            </span>
          )}
          {alert.resolved_by && (
            <span className="alert-audit-item">
              Resolved by <strong>{alert.resolved_by}</strong>
              {alert.resolved_at && ` · ${getTimeAgo(alert.resolved_at)}`}
            </span>
          )}
          {alert.glpi_ticket_id && (
            <a
              href={`https://techsupport.vinayenterprises.co.in/front/ticket.form.php?id=${alert.glpi_ticket_id}`}
              target="_blank"
              rel="noopener noreferrer"
              className="alert-ticket-link"
              onClick={e => e.stopPropagation()}
            >
              <ExternalLink className="w-3 h-3" />
              GLPI #{alert.glpi_ticket_id}
            </a>
          )}
        </div>
      )}
      ...
      <motion.div variants={stagger} initial="hidden" animate="visible" className="alerts-root">
        ...
        <motion.div variants={fadeUp} className="alerts-filters">
            ...
            {['critical', 'high', 'medium', 'low'].map(sev => {
              ...
              return (
                <button key={sev}
                  ...
                  <span style={active ? { color: s.text.replace('text-', 'var(--color-') + ')' } : {}}>{sev}</span>
                </button>
              );
            })}
          </div>
          ...
          {(filters.state || filters.severity || filters.type) && (
            <button onClick={() => setFilters({ state: '', severity: '', type: '' })} className="filter-clear">✕ Clear</button>
          )}
        </motion.div>
        ...
        <div className="alerts-list">
          <AnimatePresence mode="popLayout">
            {alerts.length === 0 ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="alerts-empty">
                <CheckCircle2 className="w-10 h-10 text-status-up opacity-50" />
                <p className="text-vemio-text-muted text-sm">
                  {!filters.state && !filters.severity && !filters.type ? 'All clear — no active alerts' : 'No alerts matching current filters'}
                </p>
              </motion.div>
            ) : (
              alerts.map(a => (
        ...
        {data?.pagination?.total > 50 && (
          <p className="alerts-pagination">Showing {alerts.length} of {data.pagination.total} alerts</p>
        )}
      </motion.div>

      <style>{`
        .alerts-root { display: flex; flex-direction: column; gap: 20px; max-width: 1400px; }
        .alerts-header { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; flex-wrap: wrap; }
        .alerts-header-actions { display: flex; align-items: center; gap: 8px; flex-shrink: 0; }
        .alerts-title { font-size: 18px; font-weight: 700; color: var(--vemio-text); margin: 0; }
        .alerts-subtitle { font-size: 13px; color: var(--vemio-text-muted); margin: 3px 0 0; }
        .alerts-readonly { color: var(--color-vemio-text-dim); font-style: italic; }
        .alerts-refresh-btn { padding: 6px 14px; font-size: 10px; text-transform: uppercase; letter-spacing: 0.08em; color: var(--vemio-text-muted); border: 1px solid var(--color-vemio-border); border-radius: 6px; background: transparent; cursor: pointer; white-space: nowrap; transition: background 0.15s; flex-shrink: 0; }
        .alerts-refresh-btn:hover { background: var(--color-vemio-surface-raised); }

        .alerts-summary { display: grid; grid-template-columns: repeat(5, 1fr); gap: 10px; }
        @media (max-width: 1023px) { .alerts-summary { grid-template-columns: repeat(3, 1fr); } }
        @media (max-width: 767px) { .alerts-summary { grid-template-columns: repeat(2, 1fr); gap: 8px; } }
        .summary-card { border-radius: 10px; padding: 12px; text-align: left; cursor: pointer; transition: background 0.15s, border-color 0.15s; background: var(--color-vemio-surface); border: 1px solid var(--color-vemio-border); }
        .summary-card--active { background: rgba(245,158,11,0.06); border-color: rgba(245,158,11,0.2); }
        .summary-card-label { font-size: 10px; color: var(--color-vemio-text-dim); text-transform: uppercase; letter-spacing: 0.07em; margin: 0; }
        .summary-card-value { font-size: 22px; font-weight: 700; font-variant-numeric: tabular-nums; margin: 4px 0 0; }
        @media (max-width: 479px) { .summary-card-value { font-size: 18px; } }

        .alerts-filters { display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
        .filters-heading { font-size: 10px; color: var(--color-vemio-text-dim); text-transform: uppercase; letter-spacing: 0.07em; margin-right: 4px; white-space: nowrap; }
        .filters-group { display: flex; align-items: center; gap: 4px; flex-wrap: wrap; }
        .filter-chip { padding: 4px 10px; font-size: 10px; text-transform: uppercase; letter-spacing: 0.06em; border-radius: 6px; cursor: pointer; transition: background 0.15s; min-height: 30px; }
        .filters-divider { color: var(--color-vemio-border); font-size: 14px; padding: 0 2px; }
        .filter-clear { padding: 4px 10px; font-size: 10px; color: var(--color-vemio-text-muted); background: transparent; border: none; cursor: pointer; }
        .filter-clear:hover { color: var(--color-vemio-text); }

        .alerts-list { display: flex; flex-direction: column; gap: 8px; }
        .alerts-empty { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 64px 0; gap: 12px; }

        .alert-row { border-radius: 12px; padding: 14px 16px; display: flex; flex-direction: column; gap: 6px; transition: background 0.15s; }
        @media (max-width: 479px) { .alert-row { padding: 12px; } }
        .alert-meta { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
        .alert-sev-badge { display: inline-flex; align-items: center; gap: 5px; padding: 2px 8px; border-radius: 4px; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; }
        .alert-sev-dot { width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; }
        .alert-sev-dot--pulse { animation: dsc-pulse 1.5s ease-in-out infinite; }
        @keyframes dsc-pulse { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.4; transform: scale(0.75); } }
        .alert-state { font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; }
        ...
        .alert-btn { padding: 4px 12px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 0.07em; border-radius: 6px; cursor: pointer; transition: opacity 0.15s; min-height: 28px; }
        .alert-btn:disabled { opacity: 0.4; cursor: not-allowed; }
        .alert-btn--ack { color: var(--vemio-amber); border: 1px solid rgba(245,158,11,0.3); background: rgba(245,158,11,0.05); }
        .alert-btn--resolve { color: var(--color-status-up); border: 1px solid rgba(20,184,166,0.3); background: rgba(20,184,166,0.05); }

        .alerts-pagination { text-align: center; font-size: 10px; color: var(--color-vemio-text-dim); text-transform: uppercase; letter-spacing: 0.07em; }
        @media (max-width: 767px) { .alerts-root { gap: 14px; } }
      `}</style>
    </>
  );
}