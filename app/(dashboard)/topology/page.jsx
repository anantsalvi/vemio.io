...
//  Hierarchical tiered campus layout with device-type
//  coloring, vendor sub-colors, search/locate, tunnel edges
// ════════════════════════════════════════════════════════
'use client';
...
/* ═══════════════════════════════════════════════════════
   DEVICE COLOR SYSTEM
   ═══════════════════════════════════════════════════════
   Two-level: type → base color, then make → variant.
   getDeviceColor(type, make) returns the final color.
   ═══════════════════════════════════════════════════════ */
...
const TYPE_COLORS = {
  ...
  printer:        '#84CC16',
  cctv:           '#14B8A6',
  access_control: '#C084FC',
  other:          '#9CA3AF',
};

const VENDOR_COLORS = {
  'firewall:Fortinet':     '#DC2626',
  'firewall:Sophos':       '#EA580C',
  'core_switch:Cisco':     '#2563EB',
  'core_switch:HP':        '#0891B2',
  'access_point:Ruckus':   '#EC4899',
  'access_point:Fortinet': '#F59E0B',
  'access_point:Aruba':    '#06B6D4',
  'access_point:Cambium':  '#8B5CF6',
  'access_point:Cisco':    '#3B82F6',
  'access_point:Netgear':  '#84CC16',
  'router:Cisco':          '#FB923C',
};
...
/* ═══════════════════════════════════════════════════════
   STATUS CONFIG (ring style — body uses device color)
   ═══════════════════════════════════════════════════════ */
...
function buildHierarchy(nodes, edges) {
  ...
  for (let t = 1; t <= 4; t++) {
    for (const node of tiers[t]) {
      ...
      for (const nbId of neighbors) {
        ...
        if (nb && assigned.has(nb.id) && nb.tier < t && nb.tier < bestTier) { bestParent = nb; bestTier = nb.tier; }
      }
      if (!bestParent) {
        for (const nbId of neighbors) {
          const nb = nodeMap.get(nbId);
          if (nb && assigned.has(nb.id) && nb.tier === t) { bestParent = nb; break; }
        }
      }
  ...
  function mark(node) { attachedIds.add(node.id); for (const c of node.children) mark(c); }
  for (const r of roots) mark(r);
  const orphans = [];
  for (const n of nodeMap.values()) if (!attachedIds.has(n.id)) orphans.push(n);
  return { roots, orphans, nodeMap, adj };
}
...
function layoutHierarchy(roots, orphans) {
  ...
  const MIN_SP = { 0: 100, 1: 90, 2: 60, 3: 36, 4: 30 };
  const PAD = 30;
  ...
  function gw(n) {
    ...
    let w;
    if (!n.children.length) w = MIN_SP[Math.min(n.tier, 4)];
    else { w = 0; for (const c of n.children) w += gw(c); w += (n.children.length - 1) * PAD; w = Math.max(w, MIN_SP[Math.min(n.tier, 4)]); }
    wc.set(n.id, w); return w;
  }

  function pos(node, lx, aw) {
    const t = Math.min(node.tier, 4);
    const cx = lx + aw / 2;
    positions.set(node.id, { ...node, x: cx, y: TIER_Y[t], radius: TIER_RADIUS[t] });
    if (!node.children.length) return;
    const cws = node.children.map(c => gw(c));
    const tw = cws.reduce((s, w) => s + w, 0) + (node.children.length - 1) * PAD;
    let cur = cx - tw / 2;
    for (let i = 0; i < node.children.length; i++) { pos(node.children[i], cur, cws[i]); cur += cws[i] + PAD; }
  }
  ...
  let cur = PAD;
  for (let i = 0; i < reord.length; i++) { pos(reord[i], cur, rws[i]); cur += rws[i] + PAD * 2; }

  if (orphans.length > 0) {
    const oy = TIER_Y[4] + 100;
    for (let i = 0; i < orphans.length; i++) {
      const o = orphans[i]; const t = Math.min(o.tier, 4);
      positions.set(o.id, { ...o, x: PAD + i * 32, y: oy, radius: TIER_RADIUS[t], isOrphan: true });
    }
  }

  let minX = Infinity, maxX = -Infinity, maxY = -Infinity;
  for (const p of positions.values()) { minX = Math.min(minX, p.x - p.radius); maxX = Math.max(maxX, p.x + p.radius); maxY = Math.max(maxY, p.y + p.radius + 30); }
  const cw = Math.max(maxX - minX + PAD * 4, 800);
  const ch = Math.max(maxY + 60, 650);
  const sx = PAD * 2 - minX;
  if (sx) for (const p of positions.values()) p.x += sx;
  return { positions, canvasWidth: cw, canvasHeight: ch };
}
...
export default function TopologyPage() {
  ...
  useEffect(() => {
    ...
    for (const e of data.edges) {
      ...
      if (s && t) eps.push({ source: s, target: t, isTunnel: TUNNEL_TYPES.has(s.type) && TUNNEL_TYPES.has(t.type) });
    }
    linkG.selectAll('path').data(eps).join('path')
      ...
      .attr('fill', 'none')
      .attr('stroke', e => e.isTunnel ? '#06B6D4' : 'rgba(148,163,184,0.12)')
      .attr('stroke-width', e => e.isTunnel ? 2 : 0.8)
      .attr('stroke-dasharray', e => e.isTunnel ? '8,4' : 'none')
      .attr('stroke-opacity', e => e.isTunnel ? 0.7 : 1);
    ...
    // Body — colored by device type + vendor
    node.append('circle').attr('class', 'node-body')
      .attr('r', d => d.radius)
    ...
    // Type abbr
    node.filter(d => d.radius >= 10).append('text')
    ...
    // Name label (tier 0-2)
    node.filter(d => d.tier <= 2).append('text')
    ...
    node.filter(d => d.tier <= 1 && d.make).append('text')
    ...
    node.on('mouseenter', (ev, d) => {
      const nId = d.id; const conn = new Set([nId]);
      const nb = adj.get(nId); if (nb) for (const x of nb) conn.add(x);
      linkG.selectAll('path')
        .attr('stroke', e => (e.source.id===nId||e.target.id===nId) ? (e.isTunnel ? '#06B6D4' : getDeviceColor(d.type, d.make)) : 'rgba(148,163,184,0.03)')
        .attr('stroke-width', e => (e.source.id===nId||e.target.id===nId) ? (e.isTunnel ? 3 : 2) : 0.4)
        .attr('stroke-opacity', e => (e.source.id===nId||e.target.id===nId) ? 1 : 0.5);
      nodeG.selectAll('g').attr('opacity', n => conn.has(n.id) ? 1 : 0.15);
    });
    node.on('mouseleave', () => {
      linkG.selectAll('path')
        .attr('stroke', e => e.isTunnel ? '#06B6D4' : 'rgba(148,163,184,0.12)')
        .attr('stroke-width', e => e.isTunnel ? 2 : 0.8).attr('stroke-opacity', e => e.isTunnel ? 0.7 : 1);
  ...
  useEffect(() => {
    ...
    ng.each(function(d) {
      ...
      if (d.id === highlightedId) {
        el.append('circle').attr('class','node-highlight-ring').attr('r',d.radius+8).attr('fill','none').attr('stroke','#FBBF24').attr('stroke-width',3).attr('stroke-opacity',1)
          .transition().duration(600).ease(d3.easeLinear).attr('r',d.radius+22).attr('stroke-opacity',0).on('end',function(){d3.select(this).remove();});
  ...
  if (selected && data) {
    const nIds = new Set();
    for (const e of data.edges) { if (e.source===selected.id) nIds.add(e.target); if (e.target===selected.id) nIds.add(e.source); }
    for (const n of data.nodes) if (nIds.has(n.id)) neighbors.push(n);
  }
  ...
  return (
    <>
      <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.06 } } }} className="tp-root">
        <motion.div variants={fadeUp} className="tp-header">
          <div>
            ...
            <p className="tp-subtitle">{data ? `${data.nodes.length} devices · ${data.edges.length} connections` : 'Loading…'}</p>
          </div>
          ...
          {data && data.nodes.length > 0 && (
            <div className="tp-legend">
              ...
              <div className="tp-legend-section">
                ...
                <span className="tp-legend-item"><span className="tp-legend-line" style={{background:'rgba(148,163,184,0.35)'}}/>LAN</span>
                <span className="tp-legend-item"><span className="tp-legend-line tp-legend-line--tunnel"/>Tunnel / P2P</span>
              </div>
              <div className="tp-legend-section">
          ...
          {selected && (
            <motion.div key="inspector" initial={{opacity:0,x:24}} animate={{opacity:1,x:0}} exit={{opacity:0,x:24}} transition={{duration:0.25}} className="tp-inspector">
              ...
              <div className="tp-insp-badges">
                <span className="tp-insp-badge" style={{background:(STATUS_CFG[selected.status]||STATUS_CFG.unknown).color+'18',color:(STATUS_CFG[selected.status]||STATUS_CFG.unknown).color}}>
                  <span className="tp-insp-dot" style={{background:(STATUS_CFG[selected.status]||STATUS_CFG.unknown).color}}/>{(STATUS_CFG[selected.status]||STATUS_CFG.unknown).label}
                </span>
                <span className="tp-insp-badge" style={{background:getDeviceColor(selected.type,selected.make)+'18',color:getDeviceColor(selected.type,selected.make)}}>
                  {TYPE_ABBR[selected.type]||'?'} · {selected.make||(selected.type||'').replace(/_/g,' ')}
                </span>
              </div>
      ...
      <style>{`
        ...
        .tp-cat-btn--active{background:rgba(245,158,11,.12);color:var(--color-vemio-amber)}
        ...
        .tp-search-input:focus{border-color:rgba(245,158,11,.3)}
        ...
        .tp-search-result:hover{background:rgba(255,255,255,.04)}
...
function Field({ label, value, mono }) {
  ...
  return <div className="tp-field"><span className="tp-field-label">{label}</span><span className={`tp-field-value ${mono?'tp-field-value--mono':''}`}>{value}</span></div>;
}