...
export const GET = withAuth(async (req, session, { params }) => {
  ...
  try {
    ...
    const deviceResult = await queryWithTenant(tenantId,
      `SELECT vemio_device_id, name FROM devices WHERE id = $1`,
      [deviceId]
    );
    if (deviceResult.rows.length === 0) {
      return Response.json({ error: 'Device not found' }, { status: 404 });
    }
    const vemioId = deviceResult.rows[0].vemio_device_id;
    ...
    const result = await queryWithTenant(tenantId,
      `SELECT
       ...
       LEFT JOIN devices sd ON sd.vemio_device_id = dn.device_id AND dn.device_id != $1 AND sd.tenant_id = dn.tenant_id
       LEFT JOIN devices nd ON nd.vemio_device_id = dn.neighbor_device_id AND dn.device_id = $1 AND nd.tenant_id = dn.tenant_id
